      ...
      >
        ...
        <div className=" lg:px-12 pb-12 pt-4 w-full lg:h-[calc(100vh-112px)] flex flex-col relative">
          <div className="h-full w-full py-2 flex justify-between">
              <div className="flex items-center justify-between w-full lg:w-fit lg:hidden ">
                <Button
                  variant="ghost"
                  onClick={() => {
                  ...
                  className=""
                >
                  <Icons.filter className="w-5 h-5 fill-gray-400 stroke-gray-400" />
                </Button>
              </div>
              <div className="flex flex-col justify-between items-center">
              <div className="filterbg p-6 border-[#F0A724] rounded-[12px] flex flex-col justify-between items-center flex-grow overflow-y-auto">
            
          <div className="flex flex-col  h-full">
            <div className="flex items-center justify-between whitespace-nowrap gap-[30px]  w-full lg:w-[230px] xl:w-[290px] 2xl:!w-[330px] ">
              <Command className="hidden lg:block rounded-lg border mb-4 bg-[#FFBD48] border-[#F0A724]">
                <div className="flex items-center px-5" cmdk-input-wrapper="">
                  <Image src="/filter/search.png" width={20} height={20} alt="/" unoptimized priority className="mr-3" />

                  <input
                    type="text"
                    className={cn(
                      "flex w-full rounded-md bg-transparent py-3.5 h-[48px]  text-sm md:text-xl outline-none font-extrabold placeholder:text-white placeholder:text-[14px] disabled:cursor-not-allowed disabled:opacity-50"
                    )}
                    value={searchTerm}
                    placeholder="Search..."
                    onChange={(e) => {
            ...
            <div className="h-full relative flex w-full flex-col lg:flex-row gap-7">
              <div
                className={cn(
                  "hidden lg:block h-full w-full lg:w-[230px] xl:w-[290px] 2xl:!w-[330px] tallXS:overflow-y-auto flex-none  rounded-2xl col-span-2 text-base lg:text-xl",
                  { block: openFilterMobile }
                )}
              >
                <div className="flex justify-between items-center">
                <span className="hidden lg:block  text-[18px] text-white">Charity</span>
                <Image src="/filter/filter.png" width={20} height={20} alt="" unoptimized priority />

                </div>
                <Separator className="hidden lg:block my-5 bg-[#E6E6E6]" />
                {/* FILTERING OPTIONS */}
                <ScrollArea className="">
                  <div className="flex items-center justify-between mb-10 gap-8">
                  
<label htmlFor="all" className="flex items-center cursor-pointer w-full justify-between">
<input
    id="all"
...
/>
    ...
    <div className="custom-checkbox">
        <div className={`${selectedType === "all" ? "checkedmark" : "uncheckedmark"}`}></div>
    </div>
</label>
                  </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                {selectedType === filter.label && (
                                  <BsCheckLg className="pt-1 text-[20px] text-white" />
                                )}
                              </div>
                            </label>
                          </div>
            ...
            <div className="flex flex-col justify-start items-center">
                    <Image src="/heart.png" width={70} height={70} unoptimized priority alt="/"/>
                    <p className="text-white text-[14px] font-bold mt-3">Terms and condition</p>
                    </div>
            </div>
              <div className="mt-4">
                <Image src="/givingblock.png" width={130} height={33} alt="/" unoptimized priority />
              </div>
            </div>
                      ...
                      {filteredData && filteredData.length > 0 && (
                        <>
                          {filteredData.map((item, index) => (
                            ...
                            className="bg-white p-5 rounded-[12px] relative" id={item.id}>
                              <div className="flex justify-center w-full">
                              <img src={item.logo} width={200} height={200} alt="/" unoptimized priority/> 
                              </div>
                              <Separator className="my-3 bg-[#FFE6BF]" />
                              <p className="text-[#4D4D4D] text-[18px]">{item.name}</p>
                              <p className={`text-[16px] font-normal text-[#AAA] ${selected == item.id && "opacity-0"}`}>{item.city}</p>
                              {selected == item.id &&
                                <motion.div 
                                initial={{opacity:0}}
                                animate={{opacity:1}}
                                ...
                                className="absolute -bottom-[25px] left-[10px] cursor-pointer">
                                  ...
                                  <Image src="/coin.png" width={60} height={60} alt="/" unoptimized />
                                  <p className="px-5 text-[20px] font-extrabold text-[#4C81FF]">Donate</p>
                                  </div>
                                </motion.div>
